...
**ORDER_SIZE=0.001 で固定運用**

- Botは「約定させない」設計のため、注文サイズは無関係
- Maker Pointsへの影響が不明な以上、リスクを最小化
- 必要がない限り 0.001 から変更しない

```
ORDER_SIZE=0.001 固定（リスク ~$7）
  ↓
段階的な運用開始
  ↓
安定稼働
...
ORDER_SIZEを増やす必要はありません：
- Botは「約定させない」設計のため、サイズは無関係
- Maker Pointsへの影響が不明な以上、0.001で固定推奨
- どうしても増やす場合は、明確な理由とリスク理解が必要
- 各段階で動作確認
...
1. **ORDER_SIZE=0.001 で固定運用**（増やす必要なし）
2. **定期的な監視**
3. **異常時は即停止**
4. **約定 = 失敗**（設計の前提）
